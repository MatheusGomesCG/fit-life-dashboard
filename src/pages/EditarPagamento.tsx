...
const EditarPagamento: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      carregarPagamento(id);
      carregarAlunos();
    }
  }, [id]);

  const carregarPagamento = async (pagamentoId: string) => {
    try {
      setIsLoading(true);
      ...
      setForm({
        ...
        mes: pagamento.mes.toString(),
        ano: pagamento.ano.toString(),
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.status) {
      newErrors.status = "Selecione o status do pagamento";
    }
